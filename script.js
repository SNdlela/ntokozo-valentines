...
const getRandomNumber = (num) => {return Math.floor(Math.random() * (num + 1));};
  ...
  const runawayButtonLogic = (button) => {
    const moveButton = function () {
      if (this.textContent.trim() === "Okay wow. Rude. Try again.") {
  ...
  noButton.addEventListener("click", () => {
    ...
    if (noClickCount < 4) {
      ...
      //no button text
      const messages = ["No","Wait—are you sure?","Ntokozooo, just a tiny yes?","Ntokozo… I thought we had something 🥺","Okay wow. Rude. Try again.",];
  
      if (noClickCount === 4) {
        const newButton = document.createElement("button");
        newButton.id = "runawayButton";
        newButton.textContent = "Okay wow. Rude. Try again.";
  ...
  yesButton.addEventListener("click", () => {
    ...
    valentineQuestion.innerHTML = `
      ...
      Congratulations!!<br>
      <span style="font-size: 20px; color: #bd1e59;">You have scored a man of your dreams for Valentine’s Day 😌❤️</span>
    `;
    valentineQuestion.style.textAlign = "center"; 
  
    //make image go boing
    const bounceImage = document.createElement("img");
    bounceImage.src = "./images/baddie.png";
